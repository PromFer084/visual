...
while (indexUsuario < usuarios.length) {
  ...
  while (indexPublicacion < usuario.publicaciones.length) {
    ...
    const publicacionActual = usuario.publicaciones[indexPublicacion];
    console.log(publicacionActual.title);

    // Chequea con un if si el type es alquiler
    if(publicacionActual.type === "alquiler"){
      console.log("es alquiler",publicacionActual)
      totalAlquileres ++;
    }
...
console.log("Total de alquileres encontrados:", totalAlquileres);
